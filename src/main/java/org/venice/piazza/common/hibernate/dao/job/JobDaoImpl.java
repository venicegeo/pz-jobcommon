...
public class JobDaoImpl implements JobDaoCustom {
	...
	private static final String STATUS_JOB_QUERY = "select * from job where data ->> 'status' = ?1 order by data #>> regexp_split_to_array(?2, E'\\\\.') %s limit ?3 offset ?4";
	private static final String STATUS_JOB_QUERY_COUNT = "select count(*) from job where data ->> 'status' = ?1";
	private static final String USERNAME_JOB_QUERY = "select * from job where data ->> 'createdBy' = ?1 order by data #>> regexp_split_to_array(?2, E'\\\\.') %s limit ?3 offset ?4";
	private static final String USERNAME_JOB_QUERY_COUNT = "select count(*) from job where data ->> 'createdBy' = ?1";
	private static final String USERNAME_AND_STATUS_JOB_QUERY = "select * from job where data ->> 'createdBy' = ?1 and data ->> 'status' = ?2 order by data #>> regexp_split_to_array(?3, E'\\\\.') %s limit ?4 offset ?5";
